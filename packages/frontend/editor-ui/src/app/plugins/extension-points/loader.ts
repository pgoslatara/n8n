...
class ExtensionLoader {
	...
	private async activate(extensionDef: ExtensionDefinition): Promise<void> {
		...
		if (extensionDef.manifest.extends) {
			for (const [extensionPointName, config] of Object.entries(extensionDef.manifest.extends)) {
				const componentName = typeof config === 'string' ? config : config.component;
				const priority = typeof config === 'object' ? (config.priority ?? 0) : 0;

				const component = extension.components?.[componentName];
				...
				if (component) {
					...
					console.warn(
						`  ! Component ${componentName} not found in extension ${extensionDef.name}`,
					);
				}
			}
		}
